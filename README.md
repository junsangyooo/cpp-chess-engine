...
6. [구현한 기능](#6-구현한-기능)
7. [컴퓨터 AI — 난이도 5단계](#7-컴퓨터-ai--난이도-5단계)
8. [디렉토리 구조](#8-디렉토리-구조)
9. [빌드 및 실행](#9-빌드-및-실행)
...
| **AI** | 난이도 Level 1~5 (랜덤 → 캡처/체크 → 회피 → negamax 탐색 → negamax+위치평가) |
...
Strategy 패턴으로 구현된 5개 난이도. 모두 `Computer` 인터페이스의 `makeMove()`를 override합니다. Level 1~3은 **1-ply 휴리스틱**(한 수 앞만 보고 규칙 기반 선택)이고, Level 4~5는 **수읽기(look-ahead)를 하는 탐색 기반** AI입니다.
...
| **Level 4** | `LevelFour` | **Negamax + Alpha-Beta 가지치기 (depth 2)** + 재료(material) 평가 — 상대의 최선 응수까지 내다보고 둠 |
| **Level 5** | `LevelFive` | **Negamax + Alpha-Beta (depth 3)** + 재료 + **piece-square table**(중앙 장악) + 폰 전진 평가 — 가장 강함 |

> **Level 4·5의 설계 원칙:** 체스 AI의 표준 알고리즘인 **negamax(minimax 변형) + alpha-beta pruning**을 사용합니다. 핵심은 엔진을 전혀 건드리지 않은 점입니다 — 가상 수읽기는 Level 2·3이 이미 쓰던 `Board::move`/`undo` 패턴을 재귀로 감싸 구현했고, 합법성은 기존 `Chess::validMove`로, 최종 착수만 `Chess::movePiece`로 처리합니다. 평가함수는 각 기물의 `getValue()`(King 10·Q 9·R 5·B/N 3·P 1)를 합산합니다.
...
│   └── level1~5.{h,cc}    # AI 난이도 구현 (1-3: 휴리스틱, 4-5: negamax 탐색)
│
└── test/                  # 시나리오 입력 파일 (*.in)
...
| 명령 | 설명 |
|---|---|
| `game <백> <흑>` | 대국 시작. 플레이어는 `human` 또는 `computer1`~`computer5` |
